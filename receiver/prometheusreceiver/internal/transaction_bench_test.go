...
func runScrapePayloadBenchmarkV2(b *testing.B, payload benchPayload) {
	...
	for i := 0; i < b.N; i++ {
		...
		if len(payload.textBytes) > 0 {
			parseAndAppendV2(b, tx, metaMap, payload.textBytes, promTextType, symbolTable, fallbackExemplarLabels)
		}
		if len(payload.protoBytes) > 0 {
			parseAndAppendV2(b, tx, metaMap, payload.protoBytes, protoType, symbolTable, fallbackExemplarLabels)
		}
